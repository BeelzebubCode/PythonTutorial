...
with pd.ExcelWriter('rov_group.xlsx', engine='openpyxl') as writer:
    ...
    worksheet.cell(row=start_row_group_b - 1, column=1, value='Group B')
    worksheet.cell(row=start_row_group_b, column=1, value='โรงเรียน')
    worksheet.cell(row=start_row_group_b, column=2, value='จังหวัด')
    worksheet.cell(row=start_row_group_b, column=3, value='ทีม')
